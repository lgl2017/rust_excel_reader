...
/// ```
// tag: lnStyleLst
pub type LineStyleList = Vec<Outline>;

pub(crate) fn load_line_style_list(reader: &mut XmlReader) -> anyhow::Result<LineStyleList> {
    ...
    loop {
        ...
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(ref e)) if e.local_name().as_ref() == b"ln" => {
                outlines.push(Outline::load(reader, e)?);
            }
