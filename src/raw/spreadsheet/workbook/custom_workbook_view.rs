...
impl XlsxCustomWorkbookView {
    pub(crate) fn load(reader: &mut XmlReader, e: &BytesStart) -> anyhow::Result<Self> {
        ...
        let mut view = Self {
            ...
            changes_saved_win: Some(false),
            guid: None,
            include_hidden_row_col: Some(true),
            include_print_settings: Some(true),
            maximized: Some(false),
            merge_interval: None,
            name: None,
            only_sync: Some(false),
            personal_view: Some(false),
            show_comments: None,
            show_formula_bar: Some(true),
